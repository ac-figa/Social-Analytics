"""
Main entrypoint: pulls YouTube channel videos + stats, reattaches manual
classifications, upserts YouTube_Master, marks vanished videos, appends
today's YouTube_Insights_History snapshot, and mirrors into the shared
...
from .youtube_client import FatalAPIError, YouTubeAPIError, YouTubeClient

# See instagramanalyticspipeline/src/pipeline.py for why this path math:
# src/ -> youtubepipeline/ -> repo root, which also contains shared/.
...
def run(full_refresh: bool = False) -> int:
    ...
    log.info("Fetching video list for channel %s ...", channel_info.get("title"))
    try:
    ...
    if not video_list:
        log.warning("No videos returned for this channel -- nothing to do.")
        return 0
    ...
    # only re-fetch stats for recently published videos. --full bypasses
    # this for a one-off backfill.
    if full_refresh:
        log.info("Full refresh requested -- re-fetching stats for all %d video(s).", len(video_list))
        ...
        refresh_items = [
            v
            for v in video_list
            if (published := transform.parse_timestamp(v.get("published_at"))) is None
            or published >= cutoff
        ]
        skipped_count = len(video_list) - len(refresh_items)
        if skipped_count:
            log.info(
                "Skipping stats refresh for %d video(s) published more than %d days ago. "
                "Run with --full to refresh everything.",
                skipped_count,
    ...
    for video_id in refresh_ids:
        try:
            ...
            row = transform.build_master_row(video=video, channel_info=channel_info)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Refresh every video's stats, ignoring INSIGHTS_REFRESH_DAYS "
