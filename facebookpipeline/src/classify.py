...
one Video_ID. Writes to facebook_classifications, which the pipeline reads
and reattaches to Facebook_Master on every refresh.

Usage:
  python -m src.classify --video-id 1234567890 --partnership "Caffe Borbone" --content-type "Sponsored Integration"
"""
import argparse
import sys
...
def main() -> int:
    ...
    parser.add_argument("--video-id", required=True, help="Facebook video ID")
    ...
    print("Run the pipeline again to reattach this to Facebook_Master, or query "
          "facebook_classifications directly -- it is already the source of truth.")
    return 0
