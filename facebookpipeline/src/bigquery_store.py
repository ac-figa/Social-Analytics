...
docstring for the reasoning (debuggable staging, idempotent MERGE,
classifications preserved via reattachment before rows reach this module).
"""
import logging
...
MASTER_TABLE = "facebook_master"
STAGING_TABLE = "facebook_master_staging"
CLASSIFICATIONS_TABLE = "facebook_classifications"
HISTORY_TABLE = "facebook_insights_history"
HISTORY_STAGING_TABLE = "facebook_insights_history_staging"
...
MASTER_SCHEMA = [
    ...
    bigquery.SchemaField("Length", "FLOAT64"),
...
def insert_history_snapshot(client: bigquery.Client, rows: list, snapshot_date: str) -> None:
    ...
    WHEN NOT MATCHED THEN INSERT (Snapshot_Date, Video_ID, Views, Views_Organic, Impressions, Likes, Comments, Watch_Time)
      VALUES (S.Snapshot_Date, S.Video_ID, S.Views, S.Views_Organic, S.Impressions, S.Likes, S.Comments, S.Watch_Time)
    """
    client.query(merge_sql).result()
